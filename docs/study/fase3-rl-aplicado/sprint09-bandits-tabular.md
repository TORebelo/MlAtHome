...
### Sessão 1 — Multi-armed bandit

Reference:    Sutton & Barto cap 2
Build:        ε-greedy, UCB1, Thompson sampling em 10-armed bandit. Plotar regret cumulativo.
Output:       01_bandits.py
Deeper:       FareedKhan 01_simple_rl.ipynb (intro RL geral, opcional)

### Sessão 2 — Markov Decision Process intuição

Reference:    Sutton & Barto cap 3-4
Build:        Implementar GridWorld 4x4. Value iteration + policy iteration manuais. Plotar value function e policy.
Output:       02_mdp_gridworld.py

### Sessão 3 — Q-learning tabular
...
### Sessão 4 — SARSA vs Q-learning vs Expected SARSA
...
### Sessão 5 — Monte Carlo control

Reference:    Sutton & Barto cap 5
Build:        MC com exploring starts em Blackjack (`gymnasium`). Plotar value function 3D.
Output:       05_mc_blackjack.py

### Sessão 6 — Dyna-Q (model-based vs model-free)
...
### Sessão 7 — Function approximation linear

Reference:    Sutton & Barto cap 9-10
Build:        Q-learning com features lineares (tile coding) em MountainCar. Mostrar generalização vs tabular.
Output:       07_linear_fa.py

### Sessão 8 — Monte Carlo Tree Search (preview Sprint 11.5)
...
### Sessão 9 — Vanilla Policy Gradient — bridge to Sprint 10
