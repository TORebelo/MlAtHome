...
4. **Honest backtesting do RL agent.**
...
6. **Failure analysis.** Build: análise qualitativa — quando o RL agent falha catastroficamente? Plot equity curve com episode highlights, drawdowns máximos, períodos onde a policy explode. Output: `06_failure_analysis.py`.
...
8. **Final report.** Build: relatório markdown comparando RL vs baselines em 3 universos (US large-cap, ETFs sector rotation, crypto top-10). Honestidade brutal sobre limitações: lookahead, costs, regime-shift, sample-size. Output: `relatorios/rl-finance-survey.md`.
