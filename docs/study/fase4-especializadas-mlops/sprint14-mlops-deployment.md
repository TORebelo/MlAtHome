...
### Sessão 1 — Estudar a arquitetura Made-With-ML
...
### Sessão 2 — Data pipeline + preprocessing
...
### Sessão 3 — Train + tune com Ray Train + Ray Tune + MLflow
...
### Sessão 4 — Serving com Ray Serve (industry reference) + comparativo FastAPI
...
### Sessão 5 — Docker containerization

Build:        Dockerfile multi-stage para a tua API FastAPI. `docker-compose.yml` com a API + Postgres + Redis.
Output:       05_docker/

### Sessão 6 — Load testing

Build:        usar `locust` para load test contra ambas (FastAPI + Ray Serve se conseguires correr local). Medir latency p50/p95/p99 e throughput.
Output:       06_load_test/

### Sessão 7 — Monitoring com Prometheus + Grafana

Reference:    Huyen cap 8 (model monitoring)
Build:        instrumentar API com `prometheus_client`. Dashboard com latency + error rate + prediction distribution.
Output:       07_monitoring/

### Sessão 8 — Data drift detection em produção

Build:        integrar `evidently` ou `nannyml` na API para alertas de drift contínuos. Reusar conhecimento Sprint 13.
Output:       08_drift_prod/

## Semana 2 — CI/CD + jobs + deployment final

### Sessão 9 — Anyscale Jobs como referência de batch workloads
...
### Sessão 10 — Feature store básico

Build:        Feast ou simple Postgres-based feature store. Demonstrar reutilização entre training e serving.
Output:       10_feature_store/

### Sessão 11 — A/B testing infrastructure

Build:        serve 2 model versions com weighted routing 50/50, comparar metrics em real-time.
Output:       11_ab_test/

### Sessão 12 — Retraining pipeline

Build:        scheduled retraining quando drift detected (cron ou GH Actions). Trigger condicional.
Output:       12_retraining/

### Sessão 13 — Final deployment

Build:        deploy real (Railway / Fly.io / Render) de um model com monitoring + URL pública. Replicar a forma do MWML mas em stack mais leve.
